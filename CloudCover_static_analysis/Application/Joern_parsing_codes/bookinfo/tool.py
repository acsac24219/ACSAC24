...
progaV2 = re.compile(r"^(val res\d*: String = \")(.*)\"$")
...
def formatJoernResultV2(result):
    parse_result=progaV2.match(result['stdout'].strip()).group(2)
    return json.loads(eval("'{}'".format(parse_result.replace("'",""))))

def formatJoernResultNew(result):
    parse_result=prog.match(result['stdout'].strip()).group(2)
    return json.loads(eval("'{}'".format(parse_result.replace("'",""))))

def formatJoernResultBySize(result):
    return json.loads(eval("'{}'".format(prog_int.match(result['stdout'].strip()).group(2))))

def formatJoernResultWithoutJson(result):
    return prog.match(result['stdout'].strip()).group(2)
